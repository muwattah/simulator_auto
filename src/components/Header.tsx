...
export default function Header({ onAdminClick }: Props) {
  ...
  const showPrice = hasConfig && (step === 'configurator' || step === 'recommendation' || step === 'quote');

  return (
    <header className="bg-white border-b border-industrial-200 sticky top-0 z-50 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="bg-brand-600 text-white p-2 rounded-lg"><Truck size={24} /></div>
          <div>
        ...
        {showPrice && (
          <div className="hidden md:flex items-center gap-6">
            ...
            <div className="text-right">
              ...
              <p className="font-bold text-lg text-brand-700">{formatPrice(includeBTW ? pricing.totalIncl : pricing.totalExcl)}</p>
            </div>
          </div>
        )}
        <button onClick={onAdminClick} className="p-2 text-industrial-400 hover:text-industrial-600 hover:bg-industrial-100 rounded-lg transition-colors" title="Admin">
          <Settings size={20} />
        </button>
      </div>
    </header>
