/**
 * Mobile detailing cargo scene — procedural assets, GLB-ready via modelUrl later.
 */
...
function CargoShell({ space, liningVariant }: { space: CargoSpaceMeters; liningVariant: string }) {
  ...
  const matFloor = useMemo(
    () =>
      new THREE.MeshStandardMaterial({
        color: isAlu ? '#9ca3af' : isFabric ? '#1c1917' : '#3f3a36',
        roughness: isAlu ? 0.4 : 0.9,
        metalness: isAlu ? 0.65 : 0.05,
      }),
    [isAlu, isFabric]
  );
  const matWall = useMemo(
    () =>
      new THREE.MeshStandardMaterial({
        color: isAlu ? '#a8b0b8' : isFabric ? '#292524' : '#c5ccd4',
        roughness: isAlu ? 0.45 : isFabric ? 0.95 : 0.75,
        metalness: isAlu ? 0.55 : 0.1,
      }),
    [isAlu, isFabric]
  );
  const matCeil = useMemo(
    () => new THREE.MeshStandardMaterial({ color: isFabric ? '#44403c' : '#dce1e6', roughness: 0.85 }),
    [isFabric]
  );
  const doorMat = useMemo(
    () =>
      new THREE.MeshStandardMaterial({
        color: isAlu ? '#94a3b8' : isFabric ? '#1c1917' : '#6b7280',
        metalness: isAlu ? 0.5 : 0.25,
        roughness: 0.5,
  ...
  return (
    <group>
      ...
      {/* open rear doors — swung outward */}
      <mesh position={[-W / 2 - 0.15, H / 2, -L / 2]} rotation={[0, Math.PI * 0.55, 0]}>
        <boxGeometry args={[W * 0.48, H * 0.95, 0.04]} />
      ...
      {/* wheel arches */}
      <mesh position={[-W / 2 + 0.2, 0.18, -L * 0.1]}>
        <boxGeometry args={[0.35, 0.35, 0.7]} />
        <meshStandardMaterial color="#5c6570" roughness={0.85} />
      </mesh>
      <mesh position={[W / 2 - 0.2, 0.18, -L * 0.1]}>
        <boxGeometry args={[0.35, 0.35, 0.7]} />
        <meshStandardMaterial color="#5c6570" roughness={0.85} />
...
function CameraController({
  ...
  useEffect(() => {
    const targets: Record<CameraPreset, { pos: [number, number, number]; look: [number, number, number] }> = {
      // Strong rear view into installation
    ...
    <>
      ...
      {placements.map((p) => (
        <ProductMeshSwitcher
          ...
          onSelect={() => onSelectProduct(selectedProductId === p.productId ? null : p.productId)}
        />
      ))}
      ...
      <OrbitControls
        ...
        target={[0, space.height * 0.35, -space.length * 0.1]}
      />
...
export default function VanScene3D({
  space,
  placements,
  liningVariant,
  selectedProductId,
  ...
  return (
    <Canvas
      shadows
      dpr={[1, 1.5]}
      camera={{ position: [0, 1.5, -4], fov: 42, near: 0.1, far: 50 }}
